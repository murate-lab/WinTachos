...
BOOL InitInstance( HINSTANCE hInstance, int nCmdShow )
{
	...
	lResult = RegCreateKeyEx(HKEY_CURRENT_USER,
		"Software\\MurateLab\\WinTachos_rx8", NULL,	"",	REG_OPTION_NON_VOLATILE,
		...
		case WM_INITDIALOG:
			{
				...
				// バージョンをセット
				sprintf_s(cWork, sizeof(cWork), "Version %1.2f", (float)VERSION);
...
void DrawDigital(HWND hWnd, HDC hDC)
{
	...
	iSpeedDisp = (int)m_fSpeedDisp;

	hInst = (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE);
	for (iDigit = 0; iDigit < 3; iDigit++) {
		if (iDigit > 0) {
			if (iSpeedDisp / pow(10.0, iDigit) < 1) break;
		}

		int iNumber = (int)(iSpeedDisp / pow(10.0, iDigit)) % 10;

		switch (m_SettingInfo.uiSize) {
		case 0:
			hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_7SEG_H_0 + iNumber));
			break;
		case 1:
			hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_7SEG_L_0 + iNumber));
			break;
		case 2:
			hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_7SEG_N_0 + iNumber));
			break;
		case 3:
			hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_7SEG_S_0 + iNumber));
			break;
		case 4:
			hBitmap = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_7SEG_T_0 + iNumber));
			break;
		default:
			hBitmap = NULL;
			break;
